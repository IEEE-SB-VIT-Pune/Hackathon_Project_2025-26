...
const ManageTeam = () => {
    ...
    const fetchTeamData = async () => {
      try {
        ...
        const userTeamSummary = allTeams.find((t) =>
          t.members.some(
            (m) => String(m.userId._id || m.userId) === String(userData._id),
          ),
        );
        ...
        const teamDetailsRes = await API.get(
          `/teams/${userTeamSummary._id}`,
          getAuthHeaders(),
        );
  ...
  const handleLeaveTeam = async () => {
    if (
      window.confirm(
        "Are you sure you want to leave this team? This action is permanent!",
      )
    ) {
      try {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => navigate(`/user/hackathon/${id}`)}
            style={{
              background: "#2563eb",
              color: "white",
              border: "none",
  ...
  const isLeader =
    currentUser && String(team.leader._id) === String(currentUser._id);
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "1.1rem", margin: 0, opacity: 0.9 }}>
              Led by <strong>{team.leader.fullName}</strong> •{" "}
              {acceptedMembers.length} / {team.maxSize} Members
            </p>
          </div>
        ...
        >
          ...
          >
            ...
            >
              <Icon name="users" size={24} className="text-primary-dark" />{" "}
              Roster
            </h2>
                ...
                {pendingMembers.map((m) => (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      {isLeader ||
                      String(m.userId._id) === String(currentUser._id) ? (
                        <>
                          <button
                            onClick={() =>
                              handleManageMember(m.userId._id, "accepted")
                            }
                            style={{
                              flex: 1,
                              background: "#10b981",
                          ...
                          <button
                            onClick={() =>
                              handleManageMember(m.userId._id, "rejected")
                            }
                            style={{
                              flex: 1,
                              background: "#fee2e2",
