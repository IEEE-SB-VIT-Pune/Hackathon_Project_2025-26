...
export default function RoleManagement() {
      ...
      setUsers((prev) =>
        prev.map((u) => {
          ...
          if (edit.hackathonId && edit.hackathonRole) {
            up.hackathonRoles = [
              ...(up.hackathonRoles || []).filter(
                (r) => r.hackathonId !== edit.hackathonId,
              ),
  ...
  const displayed = useMemo(() => {
    ...
    if (filterHackRole !== "all") {
      list = list.filter((u) =>
        (u.hackathonRoles || []).some((r) => r.role === filterHackRole),
      );
    }
    ...
    if (filterHackathon !== "all") {
      list = list.filter((u) =>
        (u.hackathonRoles || []).some(
          (r) => (r.hackathonId?._id || r.hackathonId) === filterHackathon,
        ),
      );
    }
    ...
    if (sortBy === "role_asc")
      list.sort((a, b) =>
        (a.systemRole || "").localeCompare(b.systemRole || ""),
      );
    if (sortBy === "email_asc")
  ...
  return (
    <section style={{ marginTop: "40px", position: "relative" }}>
      ...
      >
        ...
        >
          <Icon name="users" size={24} /> Role Management
        </h2>
        <span
          style={{
            fontSize: "0.85rem",
      ...
      >
        ...
        <select
          ...
          style={{ ...SELECT_STYLE, flex: "0 0 auto" }}
        >
        ...
        <select
          value={filterHackRole}
          onChange={(e) => setFilterHackRole(e.target.value)}
          style={{ ...SELECT_STYLE, flex: "0 0 auto" }}
        >
        ...
        <select
          ...
          style={{ ...SELECT_STYLE, flex: "0 0 auto", maxWidth: "210px" }}
        >
        ...
        {(search ||
          ...
          <button
            onClick={() => {
              ...
              setFilterHackRole("all");
              setFilterHackathon("all");
              setSortBy("name_asc");
            }}
            style={{
              padding: "9px 16px",
                  ...
                  >
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      ...
                      >
                        ...
                        {(user.hackathonRoles || []).slice(0, 3).map((r, i) => (
                          <Pill key={i} label={r.role} />
                        ))}
                        {(user.hackathonRoles || []).length > 3 && (
                          ...
                          >
                            +{user.hackathonRoles.length - 3} more
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <select
                        value={edit.systemRole || user.systemRole || "user"}
                        onChange={(e) =>
                          setEdit(user._id, "systemRole", e.target.value)
                        }
                        style={{ ...SELECT_STYLE, minWidth: "140px" }}
                      >
                        <option value="user">User</option>
                        <option value="mentor">Mentor / Organizer</option>
                        <option value="admin">Admin</option>
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <select
                        value={edit.hackathonId || ""}
                        onChange={(e) =>
                          setEdit(user._id, "hackathonId", e.target.value)
                        }
                        style={{ ...SELECT_STYLE, maxWidth: "180px" }}
                      >
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <select
                        value={edit.hackathonRole || ""}
                        onChange={(e) =>
                          setEdit(user._id, "hackathonRole", e.target.value)
                        }
                        disabled={!edit.hackathonId}
                        style={{
                          ...SELECT_STYLE,
                          minWidth: "130px",
                          opacity: edit.hackathonId ? 1 : 0.5,
                        }}
                      >
                    ...
                    <td style={{ padding: "13px 16px" }}>
                      <button
                        ...
                        style={{
                          ...
                          fontWeight: "bold",
                          cursor:
                            saving === user._id ? "not-allowed" : "pointer",
                          fontSize: "0.83rem",
                          whiteSpace: "nowrap",
